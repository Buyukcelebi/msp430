...
#define BASE_URL_LDR            "GET /update?key=23KT0FP5RL63VOYW&field3="  // Used in WiFiSendMessage_LDR Function to send the Data of LDR to field 3 in ThingSpeak Channel
#define BASE_URL_TMP            "GET /update?key=23KT0FP5RL63VOYW&field1="  // Used in WiFiSendMessage_TMP Function to send the Data of TMP to field 1 in ThingSpeak Channel
#define WIFI_NETWORK_SSID	"musa" // Network SSID
#define WIFI_NETWORK_PASS	"11111111" // Network Password
...
void WiFiSendMessage_LDR_TMP(int data ) // Input the ADC distance reading
{
  if(data==LDR){
    TX(BASE_URL_LDR);
    }// Send the beginning part of the request
